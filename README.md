...
| `render_resume` | Requires `candidate_approved: true` and a valid LinkedIn URL, and refuses unfilled `[N]` placeholders. Outputs **DOCX, PDF (auto-fit to 1 page), Markdown, TXT**; set `page_size` to `"a4"` for A4 PDF/DOCX output (defaults to `"letter"`). Also returns LinkedIn alignment suggestions and the final score. |
